...
type MenuContextType = {
  ...
  removeMeal: (category: keyof MenuShape, name: string) => void;
  getAverage: (category?: keyof MenuShape | 'All') => number;
};
...
export const MenuProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const removeMeal = (category: keyof MenuShape, name: string) => {
    setMenu(prev => ({
      ...prev,
      [category]: prev[category].filter(meal => meal.name !== name),
    }));
  };

  const getAverage = (category?: keyof MenuShape | 'All') => {
    const categories =
      category && category !== 'All'
        ? [category]
        : (Object.keys(menu) as Array<keyof MenuShape>);

    const allMeals = categories.flatMap(cat => menu[cat]);
    const prices = allMeals.map(m => m.price || 0);
  ...
  return (
    <MenuContext.Provider value={{ menu, addMealsBulk, addMeal, removeMeal, getAverage }}>
      {children}
